...
traindd='./'+cFile[classId]+'/'+dirFile[dirFileId]+'/train/'+hzFile[hzId]+'/sub'+str(subjid+1)
testdd='./'+cFile[classId]+'/'+dirFile[dirFileId]+'/test/'+hzFile[hzId]+'/sub'+str(subjid+1)
...
def visualize():
    ...
    with torch.no_grad():
        for batchIdx,(inputs,targets) in enumerate(visualoader):
            ...
            scipy.io.savemat('./ForCC/sub'+str(subjid)+".mat",mdict={'groundTruth':targets.data.cpu().numpy(),'predicted':outputs.data.cpu().numpy()})
    plt.show()
